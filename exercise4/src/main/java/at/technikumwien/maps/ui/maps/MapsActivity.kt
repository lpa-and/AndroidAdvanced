...
class MapsActivity : BaseActivity(), OnMapReadyCallback {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean =
        if (item.itemId == R.id.menu_refresh) {
            ...
            viewModel.syncDrinkingFountains(true)
            true
    ...
    private fun showError(exception: Exception) {
        ...
        Snackbar.make(rootLayout, R.string.snackbar_load_retry_message, Snackbar.LENGTH_INDEFINITE)
            .setAction(R.string.snackbar_load_retry_action) { viewModel.syncDrinkingFountains(true) }
            .show()
    }
}
...
class MapsViewModel(
    ...
    drinkingFountainDao: DrinkingFountainDao
) : ViewModel() {

    constructor(dependencyManager: AppDependencyManager) : this(
        dependencyManager.syncManager,
    ...
    fun syncDrinkingFountains(force: Boolean = false) {
        ...
        viewModelScope.launch {
            try {
                syncManager.syncDrinkingFountains(force)
